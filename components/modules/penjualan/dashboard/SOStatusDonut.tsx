...
import type { SOStatusCount } from "@/lib/services/sales-kpi.service";
...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-lg px-4 py-3 text-sm">
      ...
      <p className="text-slate-600">{d.value} SO</p>
    </div>
  );
};
...
export function SOStatusDonut({ data, loading, onSliceClick }: Props) {
  if (loading) return <div className="h-56 bg-slate-50 rounded-xl animate-pulse" />;
  if (!data.length) return <div className="h-56 flex items-center justify-center text-slate-400 text-sm">Tidak ada data</div>;
  ...
  return (
    <div className="h-56 relative">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none" style={{ top: "-12px" }}>
        ...
        <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">Total SO</span>
      </div>
    </div>
  );
}