...
// Simulate auto-generated faktur number
...
export function UangMukaModal({
  ...
  useEffect(() => {
    setForm(initialData ?? { ...EMPTY_FORM, noFaktur: generateAutoFaktur() });
    setSaved(isSaved);
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl border border-slate-200 overflow-hidden flex flex-col max-h-[96vh]">
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-navy-900 to-navy-600 shrink-0">
            <div>
              ...
              <p className="text-slate-400 text-xs mt-0.5">
                {isEdit ? "Perbarui data uang muka" : "Isi data uang muka di bawah ini"}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className={`relative w-10 h-5 rounded-full transition-colors shrink-0 focus:outline-none focus:ring-2 focus:ring-navy-500/30 ${
                        form.noFakturMode === "auto" ? "bg-navy-900" : "bg-slate-300"
                      }`}
                      aria-label="Toggle auto/manual faktur"
                    >
                      <span
                        className={`absolute top-0.5 left-0.5 block w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${
                          form.noFakturMode === "auto" ? "translate-x-5" : "translate-x-0"
                        }`}
                      />
                    </button>
            ...
            <div className="px-6 py-5">
              {activeTab === "uang-muka" && (
                <div className="space-y-4">
                  ...
                  {/* No Pesanan — hanya muncul jika pelanggan sudah diisi */}
                  <div
                  ...
                  {/* Total Harga Pesanan — hanya muncul jika pelanggan sudah diisi */}
                  <div
                    className={`transition-all duration-300 overflow-hidden ${
                      hasPelanggan ? "max-h-40 opacity-100" : "max-h-0 opacity-0"
                    }`}
                  >
                    <FormField label="Total Harga Pesanan" icon={<CreditCard size={14} />}>
                      <div className="relative">
                        <input
                          type="text"
                          inputMode="numeric"
                  ...
                  {/* Uang Muka */}
                  <FormField label="Uang Muka" icon={<CreditCard size={14} />} required>
                    <div className="relative">
                      <input
                        type="text"
                        inputMode="numeric"
                    ...
                    <div className="flex flex-wrap gap-6 items-center pt-1">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer text-sm text-slate-700">
                        <input
                          ...
                          onChange={(e) => set("totalTermasukPajak", e.target.checked)}
                          className="w-4 h-4 rounded border-slate-300 accent-navy-900"
                        />
                  ...
                  <div className="border-t border-slate-100 pt-4 flex justify-end gap-8 text-sm">
                    <div className="text-right">
                      <p className="text-slate-500 text-xs font-semibold uppercase tracking-wide">Sub Total</p>
              ...
              {activeTab === "info-lainnya" && (
                <div className="space-y-4">
                  <FormField label="Syarat Pembayaran" icon={<FileText size={14} />}>
                    <div className="relative">
                      <input
                        type="text"
            ...
            <div className="px-6 py-5 border-t border-slate-100 bg-slate-50/60">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Proses Ke</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  { icon: "💳", label: "Penerimaan Penjualan", desc: "Catat penerimaan dari uang muka ini", action: handleProses },
                  { icon: "🚚", label: "Pengiriman", desc: "Buat dokumen pengiriman dari SO ini", action: () => {} },
                  { icon: "🧾", label: "Faktur", desc: "Buat faktur penjualan dari SO ini", action: () => {} },
                ].map((item) => (
                  <button
                    key={item.label}
                    disabled={!saved}
                  ...
                  >
                    ...
                    <p className={`text-xs font-bold ${saved ? "text-navy-900 group-hover:text-navy-600" : "text-slate-400"}`}>
                      {item.label}
                    </p>
