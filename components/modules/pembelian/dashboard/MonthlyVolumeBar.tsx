...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-lg px-4 py-3 text-sm">
      ...
      <p className="text-indigo-600 font-bold">{payload[0].value} PO</p>
    </div>
  );
};

export function MonthlyVolumeBar({ data, loading }: Props) {
    ...
    <div className="h-56">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }} barCategoryGap="30%">
          ...
          <Bar dataKey="value" name="PO" radius={[4, 4, 0, 0]}>
            {data.map((entry, i) => (
              <Cell
                key={i}
                fill={entry.value === maxVal ? "#6366f1" : "#c7d2fe"}
              />
            ))}
          </Bar>
        </BarChart>
