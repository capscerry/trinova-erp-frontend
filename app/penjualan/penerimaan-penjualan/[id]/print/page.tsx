...
import { PenerimaanPrintDocument } from "@/components/modules/penjualan/PenerimaanPrintDocument";
...
export default function PenerimaanPrintPage() {
  ...
  useEffect(() => {
    ...
    penerimaanPenjualanService
      ...
      .then(setData)
      .catch((err) => {
        console.error("Gagal memuat print Bukti Penerimaan:", err);
        setError("Gagal memuat dokumen Bukti Penerimaan.");
      })
      .finally(() => setLoading(false));
  }, [id]);

  if (loading) return <div className="screen-state">Memuat dokumen...</div>;
  if (error || !data) return <div className="screen-state error">{error || "Data Bukti Penerimaan tidak ditemukan."}</div>;

  return (
    <>
      {/* Tampilan dokumen sendiri sudah ada di PenerimaanPrintDocument
          (class ber-prefix "pnp-"). Di sini tinggal gaya layar (toolbar,
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .pnp-page { margin: 18px auto; box-shadow: 0 24px 70px rgba(15, 23, 42, .15); }
        @page { size: A4; margin: 0; }
        @media print {
          body { background: #fff; }
          .toolbar { display: none !important; }
          .pnp-page { box-shadow: none; margin: 0; min-height: 297mm; width: 210mm; }
        }
      `}</style>
      ...
      <PenerimaanPrintDocument data={data} />
    </>
  );
}
