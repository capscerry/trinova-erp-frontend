...
import { UangMukaPrintDocument } from "@/components/modules/penjualan/UangMukaPrintDocument";
...
export default function UangMukaPrintPage() {
  ...
  useEffect(() => {
    ...
    uangMukaService
      ...
      .then(setData)
      .catch((err) => {
        console.error("Gagal memuat print Uang Muka:", err);
        setError("Gagal memuat dokumen Uang Muka.");
      })
      .finally(() => setLoading(false));
  }, [id]);

  if (loading) return <div className="screen-state">Memuat dokumen...</div>;
  if (error || !data) return <div className="screen-state error">{error || "Data Uang Muka tidak ditemukan."}</div>;

  return (
    <>
      {/* Tampilan dokumen sendiri sudah ada di UangMukaPrintDocument (class
          ber-prefix "ump-"). Di sini tinggal gaya layar (toolbar, background
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .ump-page { margin: 18px auto; box-shadow: 0 24px 70px rgba(15, 23, 42, .15); }
        @page { size: A4; margin: 0; }
        @media print {
          body { background: #fff; }
          .toolbar { display: none !important; }
          .ump-page { box-shadow: none; margin: 0; min-height: 297mm; width: 210mm; }
        }
      `}</style>
      ...
      <UangMukaPrintDocument data={data} />
    </>
  );
}
