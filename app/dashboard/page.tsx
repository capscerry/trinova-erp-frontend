...
export default function AdminDashboardPage() {
  ...
  // ── fetch ERP stats (stat cards, module bars, recent tables) ─────────────
  ...
  // ── fetch AI rankings (risk + AHP) — runs in parallel with fetchAll ─────
  const fetchRankings = async () => {
    ...
    try {
      ...
      // ── risk rows: sorted high → low risk score ─────────────────────────
    ...
    <AppShell title="Dashboard" subtitle="Overview sistem Trinova Business Suite">
      ...
      <div className="mb-3 flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold text-slate-800">Supplier Rankings — AI & AHP</p>
        </div>
        <div className="flex-1 h-px bg-slate-100" />
        <p className="text-[11px] text-slate-400">Berbasis data ERP · XGBoost Risk · AHP-TOPSIS</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4 mb-6">
        ...
        {/* AHP 4 presets — spans 4 cols as 2×2 grid */}
