 ...
 * e.g. "DP000003", "DP-3", "3" â†’ "DP-0000000003"
 */
...
function PurchaseDownPaymentInner() {
  ...
  // â”€â”€ Derived: POs eligible to receive a new DP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // -- Stable initialData for the modal --------------------------------------
  // Computed with useMemo so the object reference only changes when the actual
  // source data changes — not on every render.
  ...
  const poId = searchParams.get("po_id");
  if (!poId) return null;
  const po = purchaseOrders.find((p: any) => String(p.purchase_order_id) === poId);
  if (!po) return null;
  return {
    purchase_order_id: po.purchase_order_id,
    supplier_id: Number(po.supplier_id ?? po.supplier?.supplier_id ?? 0),
...
// eslint-disable-next-line react-hooks/exhaustive-deps
}, [editRow?.purchase_down_payment_id, searchParams.get("po_id"), purchaseOrders.length]);
  ...
  return (
    ...
    >
        ...
        try {
            ...
            // Remember the linked PO so onNavigateToGR works from edit mode too.
            const linkedPO =
              allPurchaseOrders.find(
            ...
            // Modal stays open to show "LANJUTKAN KE" — user closes via Tutup.
...
export default function PurchaseDownPaymentContent() {
  return (
    <Suspense fallback={null}>
      <PurchaseDownPaymentInner />
    </Suspense>
  );
}