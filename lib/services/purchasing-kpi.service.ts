...
const COMPLETED_STATUSES = new Set(["Processed", "Completed"]);
...
export async function fetchPurchasingKpis(
  filter: DateFilter,
  topsisScores?: Array<{ id: string; score: number }>,  // Ci scores from useSupplierRecommendations
  activeFilters?: {
    supplierId?: number;
    category?: string;
    status?: string;
  ...
  // normalise array wrappers (API may return { data: [...] } or [...])
  ...
  const sp             = norm(spRaw);
  const poDetails      = norm(poDetailsRaw);
  const supplierCats   = norm(supplierCatsRaw);

  // ── 2. Build supplier-category ID → name lookup ──────────────────────────
  // The /supplier-category endpoint returns objects with:
  //   category_id (number) and category_name (string)
  // The /supplier endpoint stores the FK as category_supplier (numeric string)
  const categoryIdToName = new Map<string, string>();
  for (const cat of supplierCats) {
    const id   = String(cat.category_id   ?? cat.category_supplier ?? "");
    const name = cat.category_name        ?? cat.nama_category      ?? "";
    if (id && name) categoryIdToName.set(id, name);
  }

  // ── 3. Build supplier lookup maps ────────────────────────────────────────
  const supplierNameMap = new Map<number, string>();
  const supplierCatMap  = new Map<number, string>();   // supplierId → resolved category name
  for (const s of suppliers) {
    const id = Number(s.supplier_id ?? s.id);
    supplierNameMap.set(id, s.supplier_name ?? s.nama ?? `Supplier ${id}`);

    // category_supplier is stored as a numeric FK string (e.g. "5").
    // Resolve it through the category master; fall back to "Others".
    const rawCat  = String(s.category_supplier ?? "");
    const catName = rawCat
      ? (categoryIdToName.get(rawCat) ?? rawCat)   // resolved name or raw if unresolvable
      : "Others";
  ...
  // Previous-month window (same duration, shifted back)
  ...
  // ── 7. KPI — Total Spend (from invoices, paid+unpaid) ────────────────────
  ...
  // Only include suppliers that have a valid, non-zero Ci score.
  ...
  // Only include suppliers with a valid, non-zero Ci score.
  ...
  // Use product categories from PO details joined with supplier-product
  const productCatMap = new Map<number, string>();
  for (const s of sp) {
    const catName = supplierCatMap.get(Number(s.supplier_id)) ?? "Others";
    productCatMap.set(Number(s.product_id), catName);
  }
  ...
  // Fallback: count by PO if no invoices in range
  ...
  // ── 23. Supplier Risk Summary — only valid-scored suppliers ───────────────
