...
export const SUPPLIER_RECOMMENDATION_CRITERIA: Criterion[] = [
  ...
  { id: "delivery_punctuality", label: "Ketepatan Waktu",  description: "Skor ketepatan berbobot: GR lebih awal menaikkan, terlambat menurunkan skor", weight: 0.2, benefit: true  },
...
export interface SupplierRecommendationDataset {
   ...
   * Contains raw criterion values for every supplier with purchase history.
   */
  alternatives: Alternative[];
  /**
   * One entry per priority preset, each containing the full ranked list.
   * Array order matches PRIORITY_PRESETS order in lib/ahp-topsis.ts.
   */
  presets: PresetRecommendation[];
}

// ─── Module-level singleton cache ────────────────────────────────────────────
// Using a promise so concurrent callers all await the same in-flight fetch,
// and subsequent callers resolve immediately from the cached value.
...
function buildAlternatives(
  ...
  for (const [sid, a] of agg.entries()) {
    ...
    // This matches the authoritative recommendation page (app/rekomendasi/page.tsx) which
    // also includes all suppliers. Excluding 0-order suppliers changes the TOPSIS
    // vector-normalisation denominators and therefore the Ci scores and rank order,
    // causing AI Insights to show a different Rank #1 than the recommendation page.

    const avgPrice =
      a.prices.length > 0 ? a.prices.reduce((s, v) => s + v, 0) / a.prices.length : 0;
...
async function fetchAndCompute(): Promise<SupplierRecommendationDataset | null> {
  ...
  const presets: PresetRecommendation[] = PRIORITY_PRESETS.map((preset) => {
    ...
    // TOPSIS returns results in the same order as alternatives; sort descending by score.
...
export interface UseSupplierRecommendationsResult {
  /** Whether the initial fetch + calculation is still in progress */
  loading: boolean;
  /** Set if the fetch failed */
  error: string | null;
  /**
   * The evaluated dataset — identical across all consumers.
 ...
 * (regardless of which component or page they belong to) receive the same
 * cached result with no additional API calls or re-computation.
 */
export function useSupplierRecommendations(): UseSupplierRecommendationsResult {
  ...
  useEffect(() => {
    ...
    // Reuse or start the singleton promise
    ...
    _cachedPromise
      ...
      .catch((err) => {
        if (!cancelled) {
          // If the promise threw, clear the cache so the next mount retries
          _cachedPromise = null;
